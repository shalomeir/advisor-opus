---
description: Consult Claude Opus for strategic advice. Proactive auto-call only for complex multi-step tasks (3+ steps, multi-file, architecture decisions) — once before committing, once before done. Key checkpoint — when an approach is decided (formal plan mode exit OR user confirms direction in conversation) and execution is about to begin (task creation, implementation), auto-call to review before proceeding. Pivot auto-call when considering a significant change of approach mid-execution (abandoning current strategy, switching solutions, reverting work). Reactive auto-call when stuck (same error 3+ times, edit-fail loop, 5+ tool calls without progress). SKIP for trivial work (single-file edits, typo fixes, obvious next steps). Manual /advise always works.
